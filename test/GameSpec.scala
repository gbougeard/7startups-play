...
class GameSpec extends Specification {
  ...
  "Game" should {
    ...
    "get random(nbPlayer + 2) communities" in {
      val coms: Set[RegularCard] = Game.addCommunities(Age3, 5)
//      val coms2: Set[RegularCard] = Game.addCommunities(Age3, 5)
...
//      coms mustNotEqual coms2
    ...
    "get no communities if not Age3" in {
      val coms: Set[RegularCard] = Game.addCommunities(Age2, 5)
      coms must beEmpty
    }


    "deal cards " in {
      val cards = Game.deal(Age1, 3)
      //      println(s"$cards")
